    ...
    <AuthContext.Provider value={{ user, loading }}>
      <AnimatePresence mode="wait">
        {!user ? (
          ...
          >
            <div className="w-full max-w-md bg-white rounded-sm p-12 shadow-2xl text-center border-t-4 border-cyan-accent relative">
              ...
              <p className="section-label mb-10">{t('auth_label')}</p>
              
              <div className="space-y-4">
                <button
                  onClick={signInWithGoogle}
                  className="w-full flex items-center justify-center gap-4 bg-navy text-white px-8 py-5 rounded-sm font-black text-xs uppercase tracking-[0.2em] hover:bg-navy/90 transition-all hover:-translate-y-1 active:translate-y-0 shadow-lg shadow-navy/20"
                >
